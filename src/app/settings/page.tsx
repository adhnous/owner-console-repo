...
<<<<<<< HEAD
  }, [pricingEnabled, showForProviders, showForSeekers, enforceAfterMonths, lockAllToPricing, lockProvidersToPricing, lockSeekersToPricing]);
=======
  }, [
    pricingEnabled,
    showForProviders,
...
<<<<<<< HEAD
  ...
  // cascade: lockAll toggles granular
...
<<<<<<< HEAD
      <div className="oc-container">
        <div className="oc-card">
          ...
          <div className="oc-skel-line" style={{ width: '85%', marginBottom: 8 }} />
          <div className="oc-skel-line sm" style={{ width: '60%' }} />
=======
    ...
    <div className="oc-container">
      <div className="oc-toolbar">
        ...
        <div className="oc-actions">
          <button className="oc-btn" onClick={load} disabled={loading || saving}>Reload</button>
          <button className="oc-btn oc-btn-primary" onClick={save} disabled={saving || !isDirty}>{saving ? 'Saving…' : 'Save changes'}</button>
        </div>
      </div>

      {error && (<div className="oc-card" style={{ borderColor: '#fecaca', background: '#fef2f2', color: '#991b1b' }}>{error}</div>)}
      {okMsg && (<div className="oc-card" style={{ borderColor: '#bbf7d0', background: '#ecfdf5', color: '#065f46' }}>{okMsg}</div>)}

      {/* KPIs */}
      <div className="oc-kpis">
        <div className="oc-kpi"><div className="label">Pricing</div><div className="value">{pricingEnabled ? 'Enabled' : 'Disabled'}</div></div>
        <div className="oc-kpi"><div className="label">Locks</div><div className="value">{lockAllToPricing || lockProvidersToPricing || lockSeekersToPricing ? 'Active' : 'Off'}</div></div>
        <div className="oc-kpi"><div className="label">Show After</div><div className="value">{enforceAfterMonths} mo</div></div>
      </div>

      {/* Visibility */}
      <div className="oc-card">
        <h3 className="oc-title" style={{ marginBottom: 8 }}>Visibility</h3>
        <div className="oc-filter-grid cols-4">
          <div className="oc-field">
            <label className="oc-label">Enable pricing features</label>
            <input type="checkbox" className="oc-switch" checked={pricingEnabled} onChange={(e) => setPricingEnabled(e.target.checked)} />
          </div>
          <div className="oc-field">
            <label className="oc-label">Show for providers</label>
            <input type="checkbox" className="oc-switch" checked={showForProviders} onChange={(e) => setShowForProviders(e.target.checked)} disabled={disabled} />
          </div>
          <div className="oc-field">
            <label className="oc-label">Show for seekers</label>
            <input type="checkbox" className="oc-switch" checked={showForSeekers} onChange={(e) => setShowForSeekers(e.target.checked)} disabled={disabled} />
          </div>
          <div />
        </div>
      </div>
      ...
      {/* Locking */}
      <div className="oc-card">
        <h3 className="oc-title" style={{ marginBottom: 8 }}>Locking</h3>
        <div className="oc-filter-grid cols-4">
          <div className="oc-field">
            <label className="oc-label">Lock all to pricing</label>
            <input type="checkbox" className="oc-switch" checked={lockAllToPricing} onChange={(e) => onToggleLockAll(e.target.checked)} disabled={disabled} />
          </div>
          <div className="oc-field">
            <label className="oc-label">Lock providers</label>
            <input type="checkbox" className="oc-switch" checked={lockProvidersToPricing} onChange={(e) => setLockProvidersToPricing(e.target.checked)} disabled={disabled || lockAllToPricing} />
          </div>
          <div className="oc-field">
            <label className="oc-label">Lock seekers</label>
            <input type="checkbox" className="oc-switch" checked={lockSeekersToPricing} onChange={(e) => setLockSeekersToPricing(e.target.checked)} disabled={disabled || lockAllToPricing} />
          </div>
          <div />
        </div>
        <div className="oc-subtle" style={{ marginTop: 8 }}>
          When locked, provider services may be demoted to pending until the plan is paid.
        </div>
      </div>

      {/* Timing */}
      <div className="oc-card">
        <h3 className="oc-title" style={{ marginBottom: 8 }}>Timing</h3>
        <div className="oc-filter-grid cols-4">
          <div className="oc-field">
            <label className="oc-label">Show pricing after (months)</label>
            <input type="number" className="oc-input" min={0} max={12} value={enforceAfterMonths}
                   onChange={(e) => setEnforceAfterMonths(Math.max(0, Math.min(12, Math.floor(Number(e.target.value) || 0))))}
                   disabled={disabled} />
          </div>
        </div>
        <div className="oc-subtle" style={{ marginTop: 8 }}>
          {enforceAfterMonths === 0 ? 'Visible immediately for all users.' : `Visible after ${enforceAfterMonths} month${enforceAfterMonths === 1 ? '' : 's'}.`}
=======
