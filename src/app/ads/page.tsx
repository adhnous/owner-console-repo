...
export default function AdsManagerPage() {
  ...
  const [fSaleItemId, setFSaleItemId] = useState("");
  ...
  async function createAd() {
    try {
      ...
      const res = await fetch('/api/ads/create', {
        ...
        body: JSON.stringify({ text: fText, textAr: fTextAr, href: fHref, linkUrl: fHref, title: fTitle, imageUrl: fImageUrl, saleItemId: fSaleItemId, color: fColor, priority: fPriority, active: fActive }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json?.error || res.statusText);
      setFText(''); setFTextAr(''); setFHref(''); setFTitle(''); setFImageUrl(''); setFSaleItemId(''); setFColor('copper'); setFPriority(0); setFActive(true);
      await load();
    } catch (e: any) {
  ...
  async function saveRow(r: Row) {
    try {
      ...
      const res = await fetch('/api/ads/update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ id: r.id, text: r.text, textAr: r.textAr, href: r.href, linkUrl: r.linkUrl || r.href, title: r.title, imageUrl: r.imageUrl, saleItemId: r.saleItemId || '', color: r.color, active: r.active, priority: r.priority }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json?.error || res.statusText);
      await load();
  ...
  async function uploadFile(file: File): Promise<string> {
    const path = `ads/${Date.now()}-${file.name.replace(/\s+/g, '_')}`;
    ...
    <div className="oc-grid">
      ...
      <div className="oc-card" style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div>
            <label className="oc-label">Title (optional)</label>
            <input className="oc-input" value={fTitle} onChange={(e) => setFTitle(e.target.value)} placeholder="Short heading" />
          </div>
          <div>
          ...
          <div>
            <label className="oc-label">Sale Item ID (optional)</label>
            <input className="oc-input" value={fSaleItemId} onChange={(e) => setFSaleItemId(e.target.value)} placeholder="e.g. sale_items doc id" />
            <div className="oc-help" style={{ marginTop: 6 }}>If provided, the sales feed will show this sale item card in the top strip.</div>
          </div>
        </div>
          ...
          <table className="oc-table">
            <thead>
              <tr>
                ...
                <th style={{ minWidth: 220 }}>Image</th>
                <th style={{ minWidth: 180 }}>Sale Item ID</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>
                    <input className="oc-input" value={r.title || ''} onChange={(e) => setField(r.id, 'title', e.target.value)} />
                  </td>
                  <td>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 8, alignItems: 'center' }}>
                      <input className="oc-input" value={r.imageUrl || ''} onChange={(e) => setField(r.id, 'imageUrl', e.target.value)} />
                      <div>
                        <input id={`file-${r.id}`} type="file" accept="image/*" style={{ display: 'none' }} onChange={async (e) => {
                          const file = e.target.files?.[0];
                          if (!file) return;
                          try { setUploadingId(r.id); const url = await uploadFile(file); setField(r.id, 'imageUrl', url); await saveRow({ ...r, imageUrl: url }); }
                          finally { setUploadingId(null); }
                        }} />
                        <button className="oc-btn" onClick={() => document.getElementById(`file-${r.id}`)?.click()} disabled={uploadingId === r.id}>Upload</button>
                      </div>
                    </div>
                    {(r.imageUrl) && (
                      <div style={{ marginTop: 6 }}>
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={r.imageUrl} alt="thumb" style={{ width: 100, height: 100, objectFit: 'cover', borderRadius: 6, border: '1px solid #e5e7eb' }} />
                      </div>
                    )}
                  </td>
                  <td>
                    <input className="oc-input" value={r.saleItemId || ''} onChange={(e) => setField(r.id, 'saleItemId', e.target.value)} />
                  </td>
                  <td>
